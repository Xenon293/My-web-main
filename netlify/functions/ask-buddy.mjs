...
export default async (request) => {
  ...
  if (!modelsResponse.ok) { console.error('Gemini models request failed:', modelsResponse.status, await modelsResponse.text()); return Response.json({ error: `Gemini key could not list available models (${modelsResponse.status}).` }, { status: 502 }); }
  const models = (await modelsResponse.json()).models || [];
  const canGenerate = (item) => item.supportedGenerationMethods?.includes('generateContent');
  const model = models.find((item) => item.name === 'models/gemini-3.6-flash' && canGenerate(item)) || models.find((item) => item.name?.includes('gemini-3.6-flash') && canGenerate(item)) || models.find((item) => item.name?.includes('flash') && canGenerate(item)) || models.find(canGenerate);
  if (!model) return Response.json({ error: 'This Gemini key has no model available for text generation.' }, { status: 502 });
  const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/${model.name}:generateContent`, { method: 'POST', headers, body: JSON.stringify({ system_instruction: { parts: [{ text: 'You are Buddy, a concise assistant for Haniel Molejon, a first-year IT student from Cebu, Philippines. Answer only about Haniel, this portfolio, his projects, learning goals, or safe web privacy. If unknown, say you do not know. Never claim access to private data, exact location, cookies, passwords, browsing history, or device files. Do not use emojis. Keep answers under 80 words.' }] }, contents: [{ role: 'user', parts: [{ text: prompt.trim() }] }] }) });
  if (!response.ok) { console.error('Gemini request failed:', response.status, await response.text()); const message = response.status === 401 || response.status === 403 ? 'Gemini rejected the API key. Check that it is an active Google AI Studio key.' : response.status === 429 ? 'Gemini quota is currently exhausted. Try again later.' : `Gemini returned an error (${response.status}).`; return Response.json({ error: message }, { status: 502 }); }
