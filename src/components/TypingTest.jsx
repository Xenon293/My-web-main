...
const WORD_BANK = [
  ...
  'not', 'on', 'she', 'at', 'by', 'this', 'we', 'you', 'do', 'but', 'his', 'from', 'say', 'her', 'or',
  'an', 'will', 'my', 'one', 'all', 'would', 'there', 'their', 'what', 'so', 'up', 'out', 'if', 'about',
  'who', 'get', 'which', 'go', 'me', 'when', 'make', 'can', 'like', 'time', 'no', 'just', 'him', 'know',
  'take', 'people', 'into', 'year', 'your', 'good', 'some', 'could', 'them', 'see', 'other', 'than',
  'then', 'now', 'look', 'only', 'come', 'its', 'over', 'think', 'also', 'back', 'after', 'use', 'two',
  'how', 'our', 'work', 'first', 'well', 'way', 'even', 'new', 'want', 'because', 'any', 'these', 'give',
  'day', 'most', 'us', 'code', 'build', 'create', 'learn', 'simple', 'fast', 'clean', 'system', 'change',
  'light', 'dark', 'focus', 'keep', 'world', 'great', 'small', 'need', 'feel', 'high', 'place', 'still',
  'point', 'hand', 'life', 'write', 'read', 'state', 'mind', 'power', 'start', 'run', 'open', 'close',
  'true', 'key', 'line', 'file', 'data', 'path', 'next', 'flow', 'form', 'show', 'case', 'free', 'real',
  'hard', 'more', 'both', 'under', 'never', 'always', 'today', 'sound', 'right', 'left', 'found', 'might',
  'while', 'last', 'thing', 'group', 'begin', 'help', 'every', 'quick', 'press', 'drive', 'tool', 'space'
];
...
export function TypingTest() {
  ...
  }, [isStarted, mode, currentWordIndex, wordsConfig, words.length, finishTest]);
  ...
  const currentWpm = Math.round((correctKeystrokes / 5) / (activeSeconds / 60)) || 0;
  const currentAccuracy = totalKeystrokes > 0
    ? Math.round((correctKeystrokes / totalKeystrokes) * 100)
    : 100;

  return (
    <section className="typing-test cookie-typing-section" aria-labelledby="typing-title">
      <div className="typing-test-inner">
        ...
        <div className="typing-header-bar">
          ...
          <div className="typing-controls">
            <button
              ...
              aria-label={isMuted ? 'Unmute typing sound effects' : 'Mute typing sound effects'}
            ...
            {mode === 'time' ? (
              [15, 30, 60].map((t) => (
                <button
                  ...
                  onClick={() => { setTimeConfig(t); setTimeLeft(t); }}
                >
                  {t}s
                </button>
              ))
        ...
        <div className="monkey-live-stats">
          <span className="live-metric timer">
            {mode === 'time' ? `${timeLeft}s` : `${Math.min(currentWordIndex, wordsConfig)} / ${wordsConfig}`}
          </span>
          ...
          <div className="monkey-results-card">
            ...
            <div className="results-grid">
              ...
              <div className="result-metric">
                ...
                <span className="metric-val">
                  {mode} {mode === 'time' ? `${timeConfig}s` : `${wordsConfig}w`}
                </span>
              </div>
              ...
              <div className="result-metric">
                ...
                <span className="metric-val">
                  {correctKeystrokes} / {totalKeystrokes - correctKeystrokes} / {totalKeystrokes}
                </span>
            ...
            <div className="results-actions">
              <button
                ...
                onClick={() => { resetTest(); setTimeout(focusInput, 50); }}
              >
                ↻ Restart Test
              </button>
              <span className="restart-hint">Tip: Press <kbd>Tab</kbd> or <kbd>Enter</kbd> to quickly restart</span>
            </div>
          </div>
        )}
